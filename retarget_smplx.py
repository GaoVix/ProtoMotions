...
def step2_package_smplx_for_retargeting(args, motion_list: List[str]) -> Tuple[int, Path]:
    ...
    temp_yaml = Path(args.output_path) / "_temp_smplx_retarget.yaml"
    temp_smplx_dir = Path(args.output_path) / 'temp_smplx'
    motions = []
    ...
    output_file = Path(args.output_path) / "smplx_for_retargeting.pt"
...
def step3_run_retarget_shell(args, smplx_pt: Path) -> int:
    ...
    if args.no_clip:
        ...
        script = project_root / "scripts" / "retarget_amass_to_robot_no_filter.sh"
