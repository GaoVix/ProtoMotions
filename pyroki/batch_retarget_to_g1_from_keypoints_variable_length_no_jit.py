...
#
"""
Batch retargeting script that processes each motion at its native length without truncation or padding.

This is a modified version of batch_retarget_to_g1_from_keypoints.py that preserves
...
def load_motion_data_variable_length(motion_path, source_type, subsample_factor):
    """Load and process motion data from a keypoints file WITHOUT truncation or padding.

    This function preserves the original sequence length of each motion.
    ...
    print(f"Original motion length: {original_raw_frames} raw frames.")

    # NO truncation or padding - keep the original length!
    processed_positions = raw_positions
    processed_orientations = raw_orientations
    processed_left_foot_contacts = raw_left_foot_contacts
    processed_right_foot_contacts = raw_right_foot_contacts
    ...
    # Calculate actual timesteps after subsampling
    num_timesteps = simplified_keypoints.shape[0]

    print(f"Processing motion with {num_timesteps} subsampled frames (original: {original_raw_frames}).")

    # Validate shapes - now uses actual length instead of fixed target
    expected_pos_shape = (num_timesteps, N_retarget + N_AUX, 3)
    expected_orient_shape = (num_timesteps, N_retarget + N_AUX, 3, 3)
    expected_contact_or_shape = (num_timesteps, 1)
    assert (
...
def save_contact_labels(
    ...
    Args:
        ...
        num_timesteps: Number of actual timesteps
    """
    # Extract contact labels (already smoothed from load_motion_data)
...
def main():
    """Main function for simplified humanoid retargeting with native sequence lengths."""
    ...
    if args.visualize:
        ...
        # Load initial motion data (variable length)
        (
            simplified_keypoints,
            keypoint_orientations,
            left_foot_contact,
